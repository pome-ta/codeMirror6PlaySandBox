        ...
        if (child.dirty) {
          if (
            ...
            if (
              !contentView ||
              (!contentView.parent &&
                contentView.constructor == child.constructor)
            )
              child.reuseDOM(next);
          }
          child.sync(track);
  ...
  if (view instanceof MarkView) {
    for (let child = view.dom.firstChild; child; child = child.nextSibling) {
      ...
      if (pos < len || (pos == len && desc.getSide() <= 0))
        return hasComp
          ? scanCompositionTree(pos, side, desc, text, enterView, fromText)
...
class MarkDecoration extends Decoration {
  constructor(spec) {
    ...
    super(
      start ? -1 /* InlineIncStart */ : 500000000 /* NonIncStart */,
  ...
  point(from, to, deco, active, openStart, index) {
    ...
    if (deco instanceof PointDecoration) {
      if (deco.block) {
        ...
        let cursorAfter =
          !view.isEditable && (from < to || deco.startSide <= 0);
...
class ViewUpdate {
  ...
  get geometryChanged() {
    return (
      this.docChanged || (this.flags & (8 /* Geometry */ | 2) /* Height */) > 0
    );
  }
  /**
...
function computeOrder(line, direction) {
  ...
  for (let i = 0, prev = outerType, prevStrong = outerType; i < len; i++) {
    ...
    if (type == 128 /* CS */) {
      ...
      let replace =
        (i && prev == 8) /* EN */ || (end < len && types[end] == 8) /* EN */
          ? prevStrong == 1 /* L */
            ? 1 /* L */
            : 8 /* EN */
          : 256; /* NI */
  ...
  for (let i = 0; i < len; i++) {
    if (types[i] == 256 /* NI */) {
      ...
      let replace =
        beforeL == afterL ? (beforeL ? 1 /* L */ : 2 /* R */) : outerType;
      for (let j = i; j < end; j++) types[j] = replace;
      i = end - 1;
    }
  }
          ...
          if (browser.gecko) {
            ...
            if (nextTo && nextTo != (1 /* Before */ | 2) /* After */) {
              ...
              if (text)
                anchor = new DOMPos(
                  text,
                  nextTo == 1 /* Before */ ? 0 : text.nodeValue.length
                );
            }
          }
...
function computeCompositionDeco(view, changes) {
  ...
  if (newTo - newFrom < text.length) {
    ...
    state.doc.sliceString(newFrom, newTo, LineBreakPlaceholder) != text
  ) {
...
handlers.mousedown = (view, event) => {
  view.observer.flush();
  if (
    view.inputState.lastTouchTime > Date.now() - 2000 &&
    getClickType(event) == 1
  )
  ...
  return {
    ...
    get(event, extend, multiple) {
      ...
      if (extend)
        ...
        startSel.ranges.some((r) => r.eq(range))
      )
...
class HeightMap {
  ...
  set outdated(value) {
    this.flags =
      (value ? 2 /* Outdated */ : 0) | (this.flags & ~2) /* Outdated */;
  }
  ...
  applyChanges(decorations, oldDoc, oracle, changes) {
    ...
    for (let i = changes.length - 1; i >= 0; i--) {
      ...
      let end =
        start.to >= toA
          ? start
          : me.lineAt(toA, QueryType.ByPosNoHeight, oldDoc, 0, 0);
      toB += end.to - toA;
      toA = end.to;
  ...
  replace(_from, _to, nodes) {
    ...
    if (
      ...
      (node instanceof HeightMapText ||
        (node instanceof HeightMapGap && node.flags & 4 /* SingleLine */)) &&
  ...
  updateHeight(oracle, offset = 0, force = false, measured) {
    ...
    if (measured && measured.from <= offset + this.length && measured.more) {
      ...
      oracle.heightChanged =
        ...
        Math.abs(singleHeight - this.lines(oracle.doc, offset).lineHeight) >=
          Epsilon;
      return result;
    } else if (force || this.outdated) {
...
class HeightMapBranch extends HeightMap {
  constructor(left, brk, right) {
    super(
      ...
      brk | (left.outdated || right.outdated ? 2 /* Outdated */ : 0)
    );
    this.left = left;
    this.right = right;
  ...
  lineAt(value, type, doc, top, offset) {
    ...
    if (left)
      return base.join(
        this.right.lineAt(rightOffset, subQuery, doc, rightTop, rightOffset)
      );
    else
      return this.left
...
class ViewState {
  ...
  update(update, scrollTarget = null) {
    ...
    if (
      ...
      this.viewport.to - this.viewport.from > 4000 /* DoubleMargin */
    )
      this.updateLineGaps(
  ...
  getViewport(bias, scrollTarget) {
    ...
    let marginTop =
      0.5 - Math.max(-0.5, Math.min(0.5, bias / 1000 /* Margin */ / 2));
  ...
  ensureLineGaps(current) {
    ...
    for (let line of this.viewportLines) {
      ...
      if (this.heightOracle.lineWrapping) {
        let marginHeight =
          (2000 /* Margin */ / this.heightOracle.lineLength) *
        ...
        viewFrom = findPosition(
          structure,
          (this.pixelViewport.left - marginWidth) / totalWidth
        );
        viewTo = findPosition(
          structure,
          (this.pixelViewport.right + marginWidth) / totalWidth
        );
      }
      let outside = [];
          ...
          gaps.push(
            find(
              ...
              (gap) =>
                gap.from >= line.from &&
                gap.to <= line.to &&
                Math.abs(gap.from - from) < 1000 /* HalfMargin */ &&
                Math.abs(gap.to - to) < 1000 /* HalfMargin */
  ...
  lineBlockAt(pos) {
    return (
      ...
      scaleBlock(
        this.heightMap.lineAt(pos, QueryType.ByPos, this.state.doc, 0, 0),
        this.scaler
      )
    );
  }
...
class BigScaler {
  constructor(doc, heightMap, viewports) {
    ...
    this.scale =
      (7000000 /* MaxDOMHeight */ - vpHeight) / (heightMap.height - vpHeight);
...
function applyDOMChange(view, start, end, typeOver) {
  ...
  if (start > -1) {
    ...
    let { from, to } = bounds;
    let selPoints =
      view.docView.impreciseHead || view.docView.impreciseAnchor
        ? []
    ...
    if (diff) {
      ...
      change = {
        ...
        to: from + diff.toA,
        insert: Text.of(
          reader.text.slice(diff.from, diff.toB).split(LineBreakPlaceholder)
        ),
      };
    }
...
class EditorView {
  ...
  measure(flush = true) {
    ...
    let refHeight =
      scrollTop > scrollHeight - clientHeight - 4 ? scrollHeight : scrollTop;
    try {
      for (let i = 0; ; i++) {
        ...
        if (this.viewState.scrollTarget) {
          ...
          let diff =
            this.viewState.lineBlockAt(refBlock.from).top - refBlock.top;
          if (diff > 1 || diff < -1) {
...
class MatchDecorator {
  ...
  constructor(config) {
    ...
    if (decorate) {
      ...
      let getDeco =
        typeof decoration == 'function' ? decoration : () => decoration;
      this.addMatch = (match, view, from, add) =>
...
const plugin = /*@__PURE__*/ ViewPlugin.fromClass(
  class {
    ...
    update(update) {
      let height =
        update.view.viewState.editorHeight - update.view.defaultLineHeight;
